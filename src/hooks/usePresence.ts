...
export function usePresence(tabId: string | null): PresenceSnapshot {
  ...
  return useMemo<PresenceSnapshot>(() => {
    if (!tabId) {
      return { tabId: "", tabIndex: 1, isLeader: false, activeTabCount: 1, peers: [] };
    }
    const isLeader = computeLeaderTabId(peers, tabId, joinedAtRef.current) === tabId;
    
    // Sort all peers including self by joinedAt, falling back to tabId to guarantee stable sort
    const allTabs = [
      ...Array.from(peers.values()),
      { tabId, joinedAt: joinedAtRef.current, lastSeen: Date.now() },
    ].sort((a, b) => a.joinedAt - b.joinedAt || a.tabId.localeCompare(b.tabId));
    
    const tabIndex = allTabs.findIndex((t) => t.tabId === tabId) + 1;
