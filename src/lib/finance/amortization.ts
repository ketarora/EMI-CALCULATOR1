 ...
 *  - a prepayment is applied to the balance BEFORE that month's interest
 *    is calculated, per the brief: "applied to the balance at the start
 *    of its month, before interest is charged that month." Applying it
 *    after the regular principal payment instead (as an earlier version
 *    of this function did) silently overcharges one month's interest on
 *    the prepaid amount — caught by checking this function's output
 *    against the brief's own worked example, which only matches exactly
 *    under the before-interest ordering.
 ...
 *  - a prepayment is capped at the balance carried into that month, so
 *    the loan simply closes early instead of going negative
 *  - if a prepayment fully closes the balance, no interest or regular
 *    principal is charged that month — the loan is already paid off
 *    before either would accrue
...
export function generateAmortizationSchedule(
  ...
  while (balance > 0.005 && month < safetyCap && principal > 0) {
    ...
    // Step 1: apply this month's prepayment to the balance FIRST.
    ...
    // Step 2: only now does interest accrue, on whatever balance remains.
    // If the prepayment above already closed the loan, this month has
    // no regular interest/principal component at all.
    let interestPaid = 0;
    let principalPaid = 0;
    if (balance > 0.005) {
