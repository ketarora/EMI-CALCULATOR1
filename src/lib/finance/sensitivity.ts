...
function buildAxis(current: number, offsets: number[], min: number, max: number): number[] {
  const values = offsets.map((offset) => Number(Math.min(Math.max(current + offset, min), max).toFixed(1)));
...
export function calculateSensitivityGrid(
  ...
  const rows: SensitivityCell[][] = tenures.map((tenure) =>
    rates.map((rate) => ({
      ...
      isCurrent: Math.abs(rate - currentRate) < 0.0001 && tenure === currentTenure,
    }))
  );

  return { rates, tenures, rows, currentRate, currentTenure };
}
