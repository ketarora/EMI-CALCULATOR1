...
export function SliderField({
  ...
  function endEdit(nextValue: number) {
    if (!isEditingRef.current) return;
    isEditingRef.current = false;
    if (nextValue !== startValueRef.current) onCommitEnd?.();
  }
  ...
  return (
    <div className="space-y-2">
      ...
      <input
        ...
        onPointerUp={(event) => endEdit(Number((event.target as HTMLInputElement).value))}
        onKeyDown={beginEdit}
        onKeyUp={(event) => endEdit(Number((event.target as HTMLInputElement).value))}
        onBlur={(event) => endEdit(Number(event.target.value))}
