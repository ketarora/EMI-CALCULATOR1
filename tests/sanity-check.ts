import { workspaceReducer, createHistorySnapshot, createInitialState } from "../src/context/workspaceReducer";
...
const hydrated = workspaceReducer(state, {
  type: "HYDRATE_STATE",
  payload: { ...createInitialState(), ui: { theme: "dark", mode: "compare" } },
});
assert(hydrated.ui.theme === "dark", "HYDRATE_STATE adopts shared theme");
assert(hydrated.ui.mode === state.ui.mode, "HYDRATE_STATE preserves this tab's local mode");

// --- Reducer: live previews stay smooth, then one explicit history point makes undo jump back once ---
state = createInitialState();
const beforeDrag = createHistorySnapshot(state);
state = workspaceReducer(state, { type: "PREVIEW_RATE", payload: 12 });
state = workspaceReducer(state, { type: "PREVIEW_RATE", payload: 13 });
state = workspaceReducer(state, { type: "PREVIEW_RATE", payload: 16 });
assert(state.history.past.length === 0, "PREVIEW_RATE does not create one undo step per drag tick");
state = workspaceReducer(state, { type: "PUSH_HISTORY", payload: beforeDrag });
state = workspaceReducer(state, { type: "UNDO" });
assert(state.calculator.rate === 11, "UNDO after a slider drag jumps directly back to the pre-drag value");
...
// Prepayment ordering: the brief is explicit that a prepayment is
// "applied to the balance at the start of its month, before interest is
// charged that month." An earlier version of generateAmortizationSchedule
// applied it AFTER that month's regular principal payment instead, which
// silently overcharged one month's interest on the prepaid amount. Pinned
// directly to the brief's own worked example (₹15,00,000 @ 11% / 48mo,
// ₹1,00,000 prepayment in month 12 -> ≈₹38,477 saved, ≈3 months reduced,
// new total interest ≈₹3,22,401) so this can't silently regress.
const prepaymentWorkedExample = generateAmortizationSchedule(15_00_000, 11, 48, [
  { id: "worked-example", month: 12, amount: 1_00_000 },
]);
const baseWorkedExample = generateAmortizationSchedule(15_00_000, 11, 48);
const interestSavedWorked = baseWorkedExample.totalInterest - prepaymentWorkedExample.totalInterest;
assert(
  Math.abs(interestSavedWorked - 38_477) < 50,
  `prepayment applied BEFORE interest accrues that month, matching the brief's worked example (got ₹${Math.round(interestSavedWorked)} saved, brief says ≈₹38,477)`
);
assert(
  baseWorkedExample.actualTenure - prepaymentWorkedExample.actualTenure === 3,
  "tenure reduced by exactly 3 months, matching the brief's worked example"
);
